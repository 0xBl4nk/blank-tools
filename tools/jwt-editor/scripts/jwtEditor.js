...
// Function to export with all event handler references for proper cleanup
function jwtEditor() {
    // DOM Elements
    ...
    async function updateJwt() {
        ...
        try {
            ...
            // After updating the token, verify it with the secret key
            if (secretKey) {
                const isValid = await verifyJwt(newJwt, secretKey);
                updateTokenStatus(isValid);
            }
    ...
    // Event handler for JWT input
    function handleJwtInput() {
        parseJwt(jwtInput.value);
    }

    // Event handler for header output
    function handleHeaderOutput() {
        isEdited = true;
        updateJwt();
    }

    // Event handler for payload output
    function handlePayloadOutput() {
        isEdited = true;
        updateJwt();
    }

    // Event handler for signature output
    async function handleSignatureOutput() {
        secretKey = signatureOutput.value.trim();
        const token = jwtInput.value;
        
        // Only verify the original input token if we're not in edit mode
        if (!isEdited && token && token.split('.').length === 3) {
            const isValid = await verifyJwt(token, secretKey);
            updateTokenStatus(isValid);
        }
        
        // If in edit mode, update the JWT
        if (isEdited) {
            await updateJwt();
        }
    }
    ...
    headerOutput.addEventListener('input', handleHeaderOutput);
    payloadOutput.addEventListener('input', handlePayloadOutput);
    signatureOutput.addEventListener('input', handleSignatureOutput);
    copyButton.addEventListener('click', handleCopyButton);

    // Parse JWT if it exists in the input on initialization
    if (jwtInput.value) {
        parseJwt(jwtInput.value);
    }
}
